...
data_name=[] #Stores names
data_ans=[]
data_ck=[] #Stores checksums
data_id=[] #Stores IDs
sdata=[] #Stores split data

for i in xrange(0,len(data)):
    sdata.append(data[i].split('-'))

for i in xrange(0,len(sdata)):
    data_ans.append(sdata[i][len(sdata[i])-1])
    data_ans[i].split('[')
    data_ck.append(data_ans[i].replace(']','').split('[')[1])
    data_id.append(data_ans[i].split('[')[0])
    
for i in xrange(0,len(sdata)):
    data_name.append(sdata[i][:len(sdata[i])-1])
...
data_order=joinorder(data_name) #Names with order
...
ans=[]
for i in xrange(0,len(data_order)):
    for k in xrange(0, len(data_ck[i])):
        ans.append(count_l(data_order[i],data_ck[i][k]))

print ans[5:10]

contador=[]
for i in xrange(0,len(ans),5):
    if ans[i]>=ans[i+1] and ans[i+1]>=ans[i+2] and ans[i+2]>=ans[i+3] and ans[i+3]>=ans[i+4]:  
        if ans[i]==ans[i+1]:
            if alfa(data_ck[i/5][0],data_ck[i/5][1])==True:
                contador.append(i)
        elif ans[i+1]==ans[i+2]:
            if alfa(data_ck[i/5][1],data_ck[i/5][2])==True:
                contador.append(i)
        elif ans[i+2]==ans[i+3]:
            if alfa(data_ck[i/5][2],data_ck[i/5][3])==True:
                contador.append(i)
        elif ans[i+3]==ans[i+4]:
            if alfa(data_ck[i/5][3],data_ck[i/5][4])==True:
                contador.append(i)
        else:
            contador.append(i)
...
soma=0
for i in xrange(0,len(contador)):
    soma=soma+int(data_id[i])
